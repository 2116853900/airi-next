...
if (!goos || !goarch) {
  console.error(`Unsupported target: os=${os} arch=${arch} (expected os in win|mac|linux, arch in x64|arm64)`)
  process.exit(1)
}

const outDir = join(engineRoot, 'out', os)
mkdirSync(outDir, { recursive: true })
const output = join(outDir, os === 'win' ? 'unibarrage.exe' : 'unibarrage')
...
console.info(`Built UniBarrage ${os}/${arch} -> ${output}`)
