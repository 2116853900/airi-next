	...
	handleDanmaku := func(event interface{}) {
		...
		data, _ := uni.CreateUniMessage(
			...
			&uni.ChatMessage{
				Name:       d.Sender.Uname,
				Avatar:     avatar,
				Level:      userLevelFromRaw(d.Raw),
				MedalLevel: medalLevelOf(d.Sender.Medal),
				Content:    d.Content,
	...
	handleGift := func(event interface{}) {
		...
		data, _ := uni.CreateUniMessage(
			...
			&uni.GiftMessage{
				Name:       g.Uname,
				Avatar:     avatar,
				MedalLevel: int64(g.MedalInfo.MedalLevel),
				Item:       g.GiftName,
				Num:        g.Num,
	...
	handleSuperChat := func(event interface{}) {
		...
		data, _ := uni.CreateUniMessage(
			...
			&uni.SuperChatMessage{
				...
				Avatar:     avatar,
				Level:      int64(sc.UserInfo.UserLevel),
				MedalLevel: int64(sc.MedalInfo.MedalLevel),
				Content:    sc.Message,
	...
	handleLike := func(event interface{}) {
		...
		data, _ := uni.CreateUniMessage(
			...
			&uni.LikeMessage{
				...
				Avatar:     avatar,
				Level:      userLevelOf(user),
				MedalLevel: int64(l.FansMedal.MedalLevel),
				Count:      1,
				Raw:        l,
			},
		)
	...
	handleInteract := func(event interface{}) {
		...
		data, _ := uni.CreateUniMessage(
			...
			&uni.EnterRoomMessage{
				...
				Avatar:     avatar,
				Level:      userLevelOf(user),
				MedalLevel: int64(e.FansMedal.MedalLevel),
				Raw:        e,
			},
		)
		ws.BroadcastToClients(data)
...
// userLevelOf 返回 FetchUserData 解析出的平台账号等级；抓取失败时为 0。
func userLevelOf(user *UserData) int64 {
	if user == nil {
		return 0
	}
	return int64(user.Card.LevelInfo.CurrentLevel)
}

// userLevelFromRaw 从弹幕原始包解析平台账号等级；旧协议无该字段时为 0。
func userLevelFromRaw(raw string) int64 {
	return gjson.Get(raw, "info.2.16.0").Int()
}

// medalLevelOf 返回粉丝勋章等级；无勋章时为 0。
func medalLevelOf(medal *message.Medal) int64 {
	if medal == nil {
		return 0
	}
	return int64(medal.Level)
}
