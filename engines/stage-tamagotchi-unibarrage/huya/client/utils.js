...
// 统一请求发送
async function fireFetch(url, opts = {}, isJson = false) {
  try {
...
// 提取html文本的目标字符串
function matchHtmlText(html, reg, defData = '') {
  if (!reg) {
    return defData
  }
 ...
 *  @return {string}
 */
function encrypt(content, algorithm = 'md5') {
 ...
 *
 */
function genUrlSearch(obj, noPrefix = false) {
  let urlQs = ''
