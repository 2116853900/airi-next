...
class huya_danmu extends events {
  ...
  async _get_chat_info() {
    try {
      const body = await fireFetch(`https://m.huya.com/${this._roomid}`, {})
      const info = {}
      // @deprecated
      ...
      if (!subsid_array || !topsid_array || !yyuid_array)
        return
      info.subsid = subsid_array[1] === '' ? 0 : Number.parseInt(subsid_array[1])
      info.topsid = topsid_array[1] === '' ? 0 : Number.parseInt(topsid_array[1])
      info.yyuid = Number.parseInt(yyuid_array[1])
      return info
    }
    catch (e) {
  ...
  _start_ws() {
    ...
    this._emitter.on('6501', (msg) => {
      if (msg.lPresenterUid != this._info.yyuid)
        return
      const gift = this._gift_info[`${msg.iItemType}`] || {
        name: '未知礼物',
        price: 0,
      }
  ...
  _bind_ws_info() {
    ...
    ws_user_info.bAnonymous = this._info.yyuid == 0
